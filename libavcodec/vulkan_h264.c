                             ...
                             int dpb_slot_index)
{
    ...
    int err = ff_vk_decode_prepare_frame(ctx, pic->f, vkpic,
                                         is_current, pic->pic_id);
    if (err < 0)
        return err;
                               ...
                               av_unused uint32_t       size)
{
    ...
    ret = vk->CreateVideoSessionParametersKHR(ctx->s.hwctx->act_dev, &session_params_create,
                                              ctx->s.hwctx->alloc, &vp->session_params);
    if (ret != VK_SUCCESS) {
    ...
    av_log(avctx, AV_LOG_DEBUG, "Created frame parameters: "
           "%i SPS %i PPS packets, %i ref slots (%i long, %i short)\n",
           h264_params_info.stdSPSCount, h264_params_info.stdPPSCount,
           vp->decode_info.referenceSlotCount, h->long_ref_count,
           h->short_ref_count);
