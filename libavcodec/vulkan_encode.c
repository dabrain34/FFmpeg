...
static AVBufferRef *vk_get_dpb_pool(FFVulkanEncodeContext *ctx)
{
                                  ...
                                  int output_delay, int decode_delay)
{
    ...
    for (i = 0; i < nb_out_fmts; i++) {
        int tmp;
        if (ff_vk_pix_fmt_from_vkfmt(ret_info[i].format, &tmp) == s->frames->sw_format) {
                            ...
                            FFVulkanEncodePicture *pic, AVFrame *src)
{
    ...
    if (!pic->dpb_ref) {
        ...
        if (ctx->layered_dpb) {
            ...
            pic->dpb_ref = av_buffer_ref(ctx->layered_frame);
        } else {
            pic->dpb_ref = vk_get_dpb_pool(ctx);
        }
        ...
        err = ff_vk_encode_create_view(ctx, &pic->dpb_view, &pic->dpb_aspect,
                                       (AVVkFrame *)pic->dpb_ref->data, layer_id);
        if (err < 0)
            goto fail;
    ...
    pic->reordered_opaque = src->reordered_opaque;
                               ...
                               AVFrame *src)
{
    ...
    /* Source image */
    err = ff_vk_exec_add_dep_frame(&ctx->s, exec, src->buf[0],
    ...
    ff_vk_exec_update_frame(&ctx->s, exec, src->buf[0], &img_bar[nb_img_bar++]);

    /* Source image's DPB */
    if (encode_info.pSetupReferenceSlot) {
        err = ff_vk_exec_add_dep_frame(&ctx->s, exec, pic->dpb_ref,
                                       VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT);
        if (err < 0)
            return err;
    }
    ...
    if (!layered_dpb) {
        for (int i = 0; i < pic->nb_refs; i++) {
            FFVulkanEncodePicture *ref = pic->refs[i];
            err = ff_vk_exec_add_dep_frame(&ctx->s, exec, ref->dpb_ref,
                                           VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT);
            if (err < 0)
                return err;
        }
...
//    pkt->duration  = pic->duration;
    ...
    avctx->reordered_opaque = pic->reordered_opaque;
                                    ...
                                    AVPacket *pkt)
{
    int err;
    AVFrame *frame;
    FFVulkanEncodePicture *cur_pic = &ctx->pic[avctx->frame_number % ctx->gop_size];
    int ft = !(avctx->frame_number % ctx->gop_size) ? FF_VK_FRAME_KEY : FF_VK_FRAME_P;

    {
        ...
        if (ft == FF_VK_FRAME_P)
            cur_pic->prev = &ctx->pic[(avctx->frame_number % ctx->gop_size) - 1];
        ...
        cur_pic->refs[0]      = !(avctx->frame_number % ctx->gop_size) ? NULL : &ctx->pic[0];
        cur_pic->slot         = avctx->frame_number % ctx->gop_size;
        cur_pic->nb_refs      = !!(avctx->frame_number % ctx->gop_size);
    ...
    if (!((avctx->frame_number + 1) % ctx->gop_size)) {
